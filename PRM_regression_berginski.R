...
for (current_file in file_name) {
  ...
  for (this_peptide in unique(df$Peptide)) {
    ...
    this_plot = ggplot(this_data_set,aes(x= Theor_conc, y= Meas_conc)) +
      ...
      labs(x='Theoretical Concentration (fmol)',y='Measured Concentration (fmol)') +
    ...
    svg(file.path(output_dir,paste0(hgnc_symbol,"-",peptide, "-FWD.svg")),width=6,height=5)
    both_plots()
    dev.off()
    convertSVGtoPNG(file.path(output_dir,paste0(hgnc_symbol,"-",peptide, "-FWD.svg")))
