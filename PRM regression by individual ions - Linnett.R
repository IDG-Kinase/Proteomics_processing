...
setwd("/home/mbergins/Documents/Projects/proteomics_processing")
current.Dir <- getwd()
...
setwd("/home/mbergins/Documents/Projects/proteomics_processing/Regression_inputs/")
...
for (current_file in file_name) {
  ...
  for (i in peptide_name) {
    ...
    if (dim(deg_f)[1] == 0) {
      print(paste0('Couldn\'t build a model for peptide: ', i, ', only found ', dim(deg_f)[1], ' samples after filtering.'))
      next;
    }
    ...
    library(MASS)
    if(length(unique(above_LOD$Theor_conc)) > 2) {
      model_robust1 = rlm(above_LOD$Meas_conc ~ above_LOD$Theor_conc)
    ...
    summary_data <- rbind(summary_data, data.frame(Peptide = i, Fragment_ions = Fragment_ions,
                                                   ...
                                                   LOB = LOB, LOD = LOD,
                                                   lm_intercept = model_intercept, lm_slope = model_slope, lm_Rsquared = model_Rsquared,
                                                   rlm_intercept = model_robust1_intercept, rlm_slope = model_robust1_slope, rlm_Rsquared = model_robust1_Rsquared,
                                                   lmRob_intercept = model_robust2_intercept, lmRob_slope = model_robust2_slope, lmRob_Rsquared = model_robust2_Rsquared,
...
setwd("/home/mbergins/Documents/Projects/proteomics_processing/Regression_inputs")
dir.create(file.path("/home/mbergins/Documents/Projects/proteomics_processing/", "Plots"), showWarnings = FALSE)
...
for (current_file in file_name) {
  ...
  ggsave(file.path("/home/mbergins/Documents/Projects/proteomics_processing/Plots", paste(filename, "png", sep=".")), p, width = 20, height = 16, units="in", dpi=600)
  ...
  ggsave(file.path("/home/mbergins/Documents/Projects/proteomics_processing/Plots", paste(filename, "png", sep=".")), p.log10, width = 20, height = 16, units="in", dpi=600)
  ...
  for (this_peptide in unique(df$Peptide)) {
    ...
    this_regression = regession_curves %>% filter(Peptide == this_peptide)
    
    uniprot_id = str_match(this_data_set$Peptide[1],"[|](.+)[|]\\w+_HUMAN\\.(.*)")[2]
    peptide = str_match(this_data_set$Peptide[1],"[|](.+)[|]\\w+_HUMAN\\.(.*)")[3]
    ...
    this_plot = ggplot(this_data_set,aes(x= Theor_conc, y= Meas_conc)) +
      ...
      labs(x='Theoretical Concentration (fmol/mg)',y='Measured Concentration (fmol/mg)') +
