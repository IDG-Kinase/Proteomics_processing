...
curve_summary_output = read_excel(here('simple_XY/PARM TABLE.xlsx'),skip=1) %>%
  ...
  mutate(`LOQ (fmol)3` = round(`LOQ (fmol)3`,2)) %>%
...
pep_seq_table = read_excel(here('simple_XY/PEP-SEQ-TAB.xlsx'),skip=1) %>%
  #toss out any protein names that are NA, these are the notes at the end of the table
  filter(!is.na(`Protein Name`)) %>%
